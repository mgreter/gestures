...
(function(a){var b={},e=[],g=0,f={};OCBNET.Gestures=function(k,d){this.el=k;this.config=a.extend({"native":{},swipeSectors:2,swipeMinDistance:15,decideOnFirst:decideScrollOrPanOnFirst},d,!0);this.id=g++;e.push(this);this.maximum=this.fingers=0;this.finger={};this.ordered=[];this.surface=f;a(k).bind("dragstart",function(){return!1});for(var c=0,b=this.init.length;c<b;c++)this.init[c].apply(this,arguments)};OCBNET.Gestures.fingerup=function(a){for(var d=e.length;d--;)e[d].fingerUp(a);b[a.id]&&(b[a.id].x=
a.x,b[a.id].y=a.y)};OCBNET.Gestures.fingermove=function(a){var d=f[a.id];if("undefined"!=typeof d){for(var c=0,h=d.length;c<h&&(d[c].fingerMove(a),!a.isPropagationStopped());c++);b[a.id].x=a.x;b[a.id].y=a.y}};OCBNET.Gestures.fingersmove=function(b){for(var d in f){var c=a.Event("fingermove",{id:d,x:b.x,y:b.y,originalEvent:b});OCBNET.Gestures.fingermove(c)}};OCBNET.Gestures.fingersup=function(b){for(var d=e.length;d--;)for(var c in e[d].finger){var h=a.Event("fingerup",{id:c,x:b.x,y:b.y,originalEvent:b});
...
c,this.start={center:this.getCenter(),rotation:this.getRotation(),distance:this.getDistance()},this.offset={x:0,y:0},this.center=this.start.center,this.distance=this.rotation=0,f[c.id]||(f[c.id]=[]),f[c.id].push(this),this.move=a.extend({},this.start))};k.fingerMove=function(d){var c=d.id;if(this.hasFinger(c)){var h=this.getFinger(c);h.x=d.x;h.y=d.y;var f=b[c].x-d.x,k=b[c].y-d.y;this.move={center:this.getCenter(),rotation:this.getRotation(),distance:this.getDistance()};this.offset={x:this.move.center.x-
this.start.center.x,y:this.move.center.y-this.start.center.y};this.center=this.move.center;this.rotation=this.move.rotation-this.start.rotation;this.distance=this.move.distance-this.start.distance;var c=Math.abs(this.offset.x),e=Math.abs(this.offset.y);d={dx:f,dy:k,finger:h,gesture:this,originalEvent:d};a(this.el).triggerHandler(new a.Event("handmove",d));"undefined"==typeof this.swipeSector&&(this.config.decideOnFirst||Math.pow(this.config.swipeMinDistance,2)<c*c+e*e)&&(h=180*Math.atan2(e,c)/Math.PI,
...
f[c]);-1!=h&&f[c].splice(h,1);0==f[c].length&&(delete b[c],delete f[c])}};k.getCenter=function(){var a={x:0,y:0},c;for(c in this.finger){var b=this.finger[c];a.x+=b.x;a.y+=b.y}this.fingers&&(a.x/=this.fingers,a.y/=this.fingers);return a};k.getRotation=function(){if(2>this.fingers)return 0;for(var a=0,b=0,h=0,f=0,e=this.ordered.length;f<e;f++)for(var k=this.finger[this.ordered[f]],g=f+1;g<e;g++){h++;var l=this.finger[this.ordered[g]],a=a+(l.x-k.x),b=b+(l.y-k.y)}return 180*Math.atan2(b/h,a/h)/Math.PI};
k.getDistance=function(){if(2>this.fingers)return 0;for(var a=0,b=0,f=0,e=0,k=this.ordered.length;e<k;e++)for(var g=this.finger[this.ordered[e]],m=e+1;m<k;m++){f++;var l=this.finger[this.ordered[m]],a=a+Math.abs(l.x-g.x),b=b+Math.abs(l.y-g.y)}a/=f;b/=f;return Math.sqrt(a*a+b*b)};k.hasFinger=function(a){return a in this.finger};k.getFinger=function(a){return this.finger[a]}})(OCBNET.Gestures.prototype);a.fn.gesture=function(b){a(this).each(function(d,c){var f=a(c).data("gesture");if("undefined"==typeof f)f=
...
(function(a){"ontouchstart"in window||window.navigator.msPointerEnabled&&0<window.navigator.msMaxTouchPoints||(function(b){b.init.push(function(b){var g=this;a(b).bind("mousedown",function(b){var e=a(b.currentTarget).data("gesture");e!==g&&eval("alert('assertion')");b=new a.Event("fingerdown",{type:"mouse",id:b.which,x:b.screenX,y:b.screenY,originalEvent:b});e.fingerDown(b)})})}(OCBNET.Gestures.prototype),a(document).bind("mouseup trapmouseup",function(b){0>b.clientX||0>b.clientY||b.clientX>a(window).width()||
...
(function(a){"ontouchstart"in window&&!(window.navigator.msPointerEnabled&&0<window.navigator.msMaxTouchPoints)&&(function(b){b.init.push(function(b){var g=this;a(b).bind("touchstart",function(b){var e=b.originalEvent.changedTouches||[],d=a(b.currentTarget).data("gesture");d!==g&&eval("alert('assertion')");a(e).each(function(c,e){var g=new a.Event("fingerdown",{type:"touch",x:e.screenX,y:e.screenY,id:e.identifier,originalEvent:b});d.fingerDown(g)})});a(b).bind("handswipe",function(a){2==a.gesture.config.swipeSectors&&
(0===a.gesture.swipeSector&&a.gesture.config["native"].panY&&a.preventDefault(),1===a.gesture.swipeSector&&a.gesture.config["native"].panX&&a.preventDefault())})})}(OCBNET.Gestures.prototype),a(document).bind("touchend touchcancel",function(b){a(b.originalEvent.changedTouches||[]).each(function(e,g){var f=a.Event("fingerup",{type:"touch",x:g.screenX,y:g.screenY,id:g.identifier,originalEvent:b});OCBNET.Gestures.fingerup(f)})}),a(document).bind("touchmove",function(b){a(b.originalEvent.changedTouches||
[]).each(function(e,g){var f=a.Event("fingermove",{type:"touch",x:g.screenX,y:g.screenY,id:g.identifier,originalEvent:b});OCBNET.Gestures.fingermove(f)})}))})(jQuery);
(function(a){if(window.navigator.msPointerEnabled&&0<window.navigator.msMaxTouchPoints){var b={up:"MSPointerUp",move:"MSPointerMove",down:"MSPointerDown"};window.navigator.pointerEnabled&&0<window.navigator.maxTouchPoints&&(b={up:"pointerup",move:"pointermove",down:"pointerdown"});(function(e){e.init.push(function(e){var f=this;a(e).bind(b.down,function(b){var d=b.originalEvent,c=a(b.currentTarget).data("gesture");c!==f&&eval("alert('assertion')");b=new a.Event("fingerdown",{type:"pointer",x:d.pageX,
y:d.pageY,id:d.pointerId,originalEvent:b});c.fingerDown(b)}).css("msTouchAction","none")})})(OCBNET.Gestures.prototype);a(document).bind(b.up,function(b){var g=b.originalEvent;b=new a.Event("fingerup",{type:"pointer",x:g.pageX,y:g.pageY,id:g.pointerId,originalEvent:b});OCBNET.Gestures.fingerup(b)});a(document).bind(b.move,function(b){var g=b.originalEvent;b=a.Event("fingermove",{type:"pointer",x:g.pageX,y:g.pageY,id:g.pointerId,originalEvent:b});OCBNET.Gestures.fingermove(b)})}})(jQuery);
