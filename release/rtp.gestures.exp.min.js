...
(function(a){var b={},c=[],g=0,h={};OCBNET.Gestures=function(b,d){this.el=b;this.config=a.extend({"native":{},swipeSectors:2,swipeMinDistance:20},d,!0);this.id=g++;c.push(this);this.maximum=this.fingers=0;this.finger={};this.ordered=[];this.surface=h;a(b).bind("dragstart",function(){return!1});for(var e=0,f=this.init.length;e<f;e++)this.init[e].apply(this,arguments)};OCBNET.Gestures.fingerup=function(a){for(var d=c.length;d--;)c[d].fingerUp(a);b[a.id]&&(b[a.id].x=a.x,b[a.id].y=a.y)};OCBNET.Gestures.fingermove=
function(a){for(var d=h[a.id],e=0,f=d.length;e<f&&(d[e].fingerMove(a),!a.isPropagationStopped());e++);b[a.id].x=a.x;b[a.id].y=a.y};OCBNET.Gestures.fingersmove=function(b){for(var d in h){var e=a.Event("fingermove",{id:d,x:b.x,y:b.y,originalEvent:b});OCBNET.Gestures.fingermove(e)}};OCBNET.Gestures.fingersup=function(b){for(var d=c.length;d--;)for(var e in c[d].finger){var f=a.Event("fingerup",{id:e,x:b.x,y:b.y,originalEvent:b});c[d].fingerUp(f)}};OCBNET.Gestures.Finger=function(a){this.id=a.id;this.x=
a.x;this.y=a.y};(function(c){c.init=[];c.fingerDown=function(d){var e=new OCBNET.Gestures.Finger(d);"undefined"==typeof b[d.id]&&(b[d.id]=new OCBNET.Gestures.Finger(d));d=new a.Event("handstart",{dx:0,dy:0,finger:e,gesture:this,originalEvent:d});a(this.el).triggerHandler(d);d.isDefaultPrevented()||this.hasFinger(e.id)||(this.start={center:this.getCenter(),rotation:this.getRotation(),distance:this.getDistance()},this.offset={x:0,y:0},this.center=this.start.center,this.distance=this.rotation=0,this.fingers++,
this.maximum<this.fingers&&(this.maximum=this.fingers),this.ordered.push(e.id),this.finger[e.id]=e,h[e.id]||(h[e.id]=[]),h[e.id].push(this),this.move=a.extend({},this.start))};c.fingerMove=function(d){var e=d.id;if(this.hasFinger(e)){var f=this.getFinger(e);f.x=d.x;f.y=d.y;var h=b[e].x-d.x,c=b[e].y-d.y;this.move={center:this.getCenter(),rotation:this.getRotation(),distance:this.getDistance()};this.offset={x:this.move.center.x-this.start.center.x,y:this.move.center.y-this.start.center.y};this.center=
this.move.center;this.rotation=this.move.rotation-this.start.rotation;this.distance=this.move.distance-this.start.distance;var e=Math.abs(this.offset.x),g=Math.abs(this.offset.y);d={dx:h,dy:c,finger:f,gesture:this,originalEvent:d};a(this.el).triggerHandler(new a.Event("handmove",d));"undefined"==typeof this.swipeSector&&Math.sqrt(e*e+g*g)>this.config.swipeMinDistance&&(f=180*Math.atan2(g,e)/Math.PI,this.swipeSector=parseInt(f/90*this.config.swipeSectors,10),this.swipeSector=Math.min(this.swipeSector,
this.config.swipeSectors-1));"undefined"!=typeof this.swipeSector&&a(this.el).triggerHandler(new a.Event("handswipe",d));1<this.fingers&&a(this.el).triggerHandler(new a.Event("handtransform",d))}};c.fingerUp=function(d){var e=d.id;if(this.hasFinger(e)){var f=this.getFinger(e);f.x=d.x;f.y=d.y;var c=b[e].x-d.x;d=b[e].y-d.y;this.stop={center:this.getCenter(),rotation:this.getRotation(),distance:this.getDistance()};this.offset={x:this.stop.center.x-this.start.center.x,y:this.stop.center.y-this.start.center.y};
this.center=this.stop.center;this.rotation=this.stop.rotation-this.start.rotation;this.distance=this.stop.distance-this.start.distance;c=new a.Event("handstop",{dx:c,dy:d,finger:f,gesture:this});a(this.el).triggerHandler(c);this.fingers--;delete this.offset;delete this.rotation;delete this.distance;delete this.swipeSector;f=a.inArray(f.id,this.ordered);this.ordered.splice(f,1);delete this.finger[e];f=a.inArray(this,h[e]);-1!=f&&h[e].splice(f,1);0==h[e].length&&(delete b[e],delete h[e])}};c.getCenter=
function(){var a={x:0,y:0},b;for(b in this.finger){var f=this.finger[b];a.x+=f.x;a.y+=f.y}this.fingers&&(a.x/=this.fingers,a.y/=this.fingers);return a};c.getRotation=function(){if(2>this.fingers)return 0;for(var a=0,b=0,f=0,h=0,c=this.ordered.length;h<c;h++)for(var g=this.finger[this.ordered[h]],k=h+1;k<c;k++){f++;var l=this.finger[this.ordered[k]],a=a+(l.x-g.x),b=b+(l.y-g.y)}return 180*Math.atan2(b/f,a/f)/Math.PI};c.getDistance=function(){if(2>this.fingers)return 0;for(var a=0,b=0,h=0,c=0,g=this.ordered.length;c<
g;c++)for(var k=this.finger[this.ordered[c]],m=c+1;m<g;m++){h++;var l=this.finger[this.ordered[m]],a=a+Math.abs(l.x-k.x),b=b+Math.abs(l.y-k.y)}a/=h;b/=h;return Math.sqrt(a*a+b*b)};c.hasFinger=function(a){return a in this.finger};c.getFinger=function(a){return this.finger[a]}})(OCBNET.Gestures.prototype);a.fn.gesture=function(b){a(this).each(function(h,c){var f=a(c).data("gesture");if("undefined"==typeof f)f=new OCBNET.Gestures(c,b),a(c).data("gesture",f);else{if(0<f.fingers)throw"fatal: live config change";
a.extend(f.config,b,!0)}});return this}})(jQuery);
