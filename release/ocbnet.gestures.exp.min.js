...
(function(a){"ontouchstart"in window&&!(window.navigator.msPointerEnabled&&0<window.navigator.msMaxTouchPoints)&&(function(b){b.init.push(function(b){var g=this;a(b).bind("touchstart",function(b){var f=b.originalEvent.changedTouches||[],d=a(b.currentTarget).data("gesture");d!==g&&eval("alert('assertion')");a(f).each(function(c,h){var f=new a.Event("fingerdown",{type:"touch",x:h.clientX||h.screenX,y:h.clientY||h.screenY,id:h.identifier,originalEvent:b});d.fingerDown(f)});d.config.hatchTouchDown||b.preventDefault()});
a(b).bind("handswipe",function(a){2==a.gesture.config.swipeSectors&&(0===a.gesture.swipeSector&&a.gesture.config["native"].panY&&a.preventDefault(),1===a.gesture.swipeSector&&a.gesture.config["native"].panX&&a.preventDefault())})})}(OCBNET.Gestures.prototype),a(document).bind("touchend touchcancel",function(b){a(b.originalEvent.changedTouches||[]).each(function(k,g){var e=a.Event("fingerup",{type:"touch",x:g.clientX||g.screenX,y:g.clientY||g.screenY,id:g.identifier,originalEvent:b});OCBNET.Gestures.fingerup(e)})}),
a(document).bind("touchmove",function(b){a(b.originalEvent.changedTouches||[]).each(function(k,g){var e=a.Event("fingermove",{type:"touch",x:g.clientX||g.screenX,y:g.clientY||g.screenY,id:g.identifier,originalEvent:b});OCBNET.Gestures.fingermove(e)})}))})(jQuery);
...
cancel:"pointercancel",action:"touch-action"});(function(d){d.init.push(function(c){var d=[],f=this.config["native"]||{};f.panY&&d.push("pan-y");f.panX&&d.push("pan-x");0==d.length&&d.push("none");f=a.proxy(b,this);a(c).bind(e.down,f).css(e.action,d.join(" "))})})(OCBNET.Gestures.prototype);var f=[e.up,e.cancel].join(" ");a(document).bind(f,g);a(document).bind(e.move,k)}})(jQuery);
