...
a.x,d[a.id].y=a.y)};OCBNET.Gestures.fingermove=function(a){for(var b=f[a.id],e=0,c=b.length;e<c&&(b[e].fingerMove(a),!a.isPropagationStopped());e++);d[a.id].x=a.x;d[a.id].y=a.y};OCBNET.Gestures.fingersmove=function(a){for(var b in f){var e=c.Event("fingermove",{id:b,x:a.x,y:a.y,originalEvent:a});OCBNET.Gestures.fingermove(e)}};OCBNET.Gestures.fingersup=function(a){for(var b=g.length;b--;)for(var e in g[b].finger){var l=c.Event("fingerup",{id:e,x:a.x,y:a.y,originalEvent:a});g[b].fingerUp(l)}};OCBNET.Gestures.Finger=
function(a){this.id=a.id;this.x=a.x;this.y=a.y};(function(a){a.init=[];a.fingerDown=function(b){var e=new OCBNET.Gestures.Finger(b);"undefined"==typeof d[b.id]&&(d[b.id]=new OCBNET.Gestures.Finger(b));b=new c.Event("handstart",{dx:0,dy:0,finger:e,gesture:this,originalEvent:b});c(this.el).triggerHandler(b);b.isDefaultPrevented()||this.hasFinger(e.id)||(this.fingers++,this.maximum<this.fingers&&(this.maximum=this.fingers),this.ordered.push(e.id),this.finger[e.id]=e,this.start={center:this.getCenter(),
rotation:this.getRotation(),distance:this.getDistance()},this.offset={x:0,y:0},this.center=this.start.center,this.distance=this.rotation=0,f[e.id]||(f[e.id]=[]),f[e.id].push(this),this.move=c.extend({},this.start))};a.fingerMove=function(b){var e=b.id;if(this.hasFinger(e)){var a=this.getFinger(e);a.x=b.x;a.y=b.y;var f=d[e].x-b.x,g=d[e].y-b.y;this.move={center:this.getCenter(),rotation:this.getRotation(),distance:this.getDistance()};this.offset={x:this.move.center.x-this.start.center.x,y:this.move.center.y-
this.start.center.y};this.center=this.move.center;this.rotation=this.move.rotation-this.start.rotation;this.distance=this.move.distance-this.start.distance;var e=Math.abs(this.offset.x),h=Math.abs(this.offset.y);b={dx:f,dy:g,finger:a,gesture:this,originalEvent:b};c(this.el).triggerHandler(new c.Event("handmove",b));"undefined"==typeof this.swipeSector&&(this.config.decideOnFirst||Math.pow(this.config.swipeMinDistance,2)<e*e+h*h)&&(a=180*Math.atan2(h,e)/Math.PI,this.swipeSector=parseInt(a/90*this.config.swipeSectors,
10),this.swipeSector=Math.min(this.swipeSector,this.config.swipeSectors-1));"undefined"!=typeof this.swipeSector&&c(this.el).triggerHandler(new c.Event("handswipe",b));1<this.fingers&&c(this.el).triggerHandler(new c.Event("handtransform",b))}};a.fingerUp=function(b){var e=b.id;if(this.hasFinger(e)){var a=this.getFinger(e);a.x=b.x;a.y=b.y;var g=d[e].x-b.x;b=d[e].y-b.y;this.stop={center:this.getCenter(),rotation:this.getRotation(),distance:this.getDistance()};this.offset={x:this.stop.center.x-this.start.center.x,
y:this.stop.center.y-this.start.center.y};this.center=this.stop.center;this.rotation=this.stop.rotation-this.start.rotation;this.distance=this.stop.distance-this.start.distance;g=new c.Event("handstop",{dx:g,dy:b,finger:a,gesture:this});c(this.el).triggerHandler(g);this.fingers--;delete this.offset;delete this.rotation;delete this.distance;delete this.swipeSector;a=c.inArray(a.id,this.ordered);this.ordered.splice(a,1);delete this.finger[e];a=c.inArray(this,f[e]);-1!=a&&f[e].splice(a,1);0==f[e].length&&
(delete d[e],delete f[e])}};a.getCenter=function(){var b={x:0,y:0},a;for(a in this.finger){var c=this.finger[a];b.x+=c.x;b.y+=c.y}this.fingers&&(b.x/=this.fingers,b.y/=this.fingers);return b};a.getRotation=function(){if(2>this.fingers)return 0;for(var b=0,a=0,c=0,f=0,d=this.ordered.length;f<d;f++)for(var g=this.finger[this.ordered[f]],h=f+1;h<d;h++){c++;var k=this.finger[this.ordered[h]],b=b+(k.x-g.x),a=a+(k.y-g.y)}return 180*Math.atan2(a/c,b/c)/Math.PI};a.getDistance=function(){if(2>this.fingers)return 0;
...
f);else{if(0<f.fingers)throw"fatal: live config change";c.extend(f.config,a,!0)}});return this}})(jQuery);
